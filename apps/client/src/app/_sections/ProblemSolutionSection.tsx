...
const PROBS = ["minOrder", "approvals", "quality", "hidden"] as const;
const SOLS = ["minOrder", "editor", "samples", "price"] as const;
    ...
    <section className="bg-background overflow-hidden" aria-labelledby="problem-heading">
      <div className="grid lg:grid-cols-2">
        ...
        >
            ...
            <ul className="space-y-3" aria-label={t("problemsLabel")}>
              {PROBS.map((key, i) => (
                <motion.li key={key}
          ...
          >
            ...
            <ul className="space-y-3 mb-10" aria-label={t("solutionsLabel")}>
              {SOLS.map((key, i) => (
                <motion.li key={key}
                  initial={{ opacity: 0, x: 12 }}
                  animate={isInView ? { opacity: 1, x: 0 } : {}}
            ...
            <Link href="#catalog"
              ...
              {t("ctaLabel")} <ArrowRight className="w-4 h-4" aria-hidden="true" />
            </Link>
          </motion.div>
      </div>
    </section>
